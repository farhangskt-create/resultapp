...
st.markdown("<p style='text-align: center;'><b>Smart Indexed Gazette Search System</b></p>", unsafe_allow_html=True)

pdf_path = "gazette.pdf"
chunks_dir = "gazette_chunks"

# 1. پی ڈی ایف کو اسمارٹ چنکس (حصوں) میں تقسیم کرنے کا فنکشن
def create_smart_chunks(pdf_file, output_dir):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
        with st.spinner("Processing gazette into secure smart indexes (One-time process)..."):
                        ...
                        for line in text.split('\n'):
                            ...
                            if line_clean:
                                ...
                                if words and words[0].isdigit() and len(words[0]) >= 3:
                                    # رول نمبر کے پہلے 2 یا 3 ہندسوں کی بنیاد پر فائل کا نام بنانا (Prefix Indexing)
                                    prefix = words[0][:3] 
                                    chunk_file = os.path.join(output_dir, f"chunk_{prefix}.txt")
                                    with open(chunk_file, "a", encoding="utf-8") as cf:
                                       cf.write(line_clean + "\n")
                                else:
                                    # دیگر معلومات یا ہیڈنگز کے لیے جنرل فائل
                                    chunk_file = os.path.join(output_dir, "chunk_general.txt")
                                    with open(chunk_file, "a", encoding="utf-8") as cf:
                                       cf.write(line_clean + "\n")

# چنکس تیار کرنا اگر پہلے سے نہ بنے ہوں
if os.path.exists(pdf_path):
    create_smart_chunks(pdf_path, chunks_dir)
    ...
    if search_option == "Search by Roll Number":
        ...
        if st.button("Search Result", type="primary"):
            if roll_input:
                ...
                with st.spinner("Searching targeted index..."):
                    # صرف متعلقہ چھوٹی سی فائل کو چیک کرنا
                    ...
                    # اگر سپیسیفک چنک میں نہ ملے تو جنرل فائل میں دیکھنا
                    if not found:
        ...
        if st.button("Search School Gazette List", type="primary"):
            if school_input:
                school_records = []
                with st.spinner("Searching school records across indexes..."):
